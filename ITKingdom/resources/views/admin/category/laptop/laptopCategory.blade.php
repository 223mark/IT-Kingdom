...
@section('content')
    <div class="container">
        ...
        <div class="row">
            <div class="col-lg-3">
                <form action="{{ route('admin#addlaptopCategory') }}" method="POST">
                    @csrf
                    <div class="card">
                        <div class="card-header">
                            <h6>Laptop Category</h6>
                        </div>
                        <div class="card-body">
                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        @foreach ($brandData as $brand)
                            <tr>
                                <th scope="row">{{ $brand->laptopbrand_id }}</th>
                                <td>{{ $brand->brand_name }}</td>
                                <td>{{ $brand->created_at->format('d/m/Y') }}</td>
                                <td>{{ $brand->updated_at->format('d/m/Y') }}</td>
                                <td>
                                    <a href="{{ route('admin#laptopCategoryUpdatePage', $brand->laptopbrand_id) }}">
